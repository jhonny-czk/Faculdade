...
bool enqueue(Queue* queue, int element) {
    // Se o indice de fim estiver logo "atras" do 
   //indice de inicio, a fila esta cheia.
...
int rear(const Queue* queue) {
    ...
    // Cuida para "apontar" para a posicao anterior do indice queue_rear, que 
    // no caso de ter "dado a volta" no vetor (voltando a 0), faz com que 
    // o ultimo elemento (fim da fila) esteja na ultima posicao do vetor (max_size)
...
void print(const Queue* queue) {
    ...
    while (i != queue->queue_rear) {
        ...
        if (i == queue->max_size) { // verifica: esta na ultima posicao?
            i = 0;
        } else {
            ++i;
        }
